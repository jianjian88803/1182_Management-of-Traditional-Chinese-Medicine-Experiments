  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(255, 255, 255, 0.18);borderRadius:15px">
        <h1 class="h1" style="color:rgba(64, 158, 255, 1);fontSize:28px;">中药实验管理系统注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			...
			<!-- <div v-if="tableName=='shiyanyuan'" class="input-group">
			   <div class="label">账号</div>
			   <div class="input-container">
			     <input v-model="ruleForm.zhanghao" class="input" type="text" placeholder="账号">
			   </div>
			 </div> -->
			<el-form-item label="账号" class="input" v-if="tableName=='shiyanyuan'">
			...
			<el-form-item label="密码" class="input" v-if="tableName=='shiyanyuan'">
			  <el-input v-model="ruleForm.mima" autocomplete="off" placeholder="密码" type="password"#elsetype="text" />
			</el-form-item>
			<!-- <div v-if="tableName=='shiyanyuan'" class="input-group">
			...
			<el-form-item label="姓名" class="input" v-if="tableName=='shiyanyuan'">
			...
			<el-form-item label="邮箱" class="input" v-if="tableName=='shiyanyuan'">
			  <el-input v-model="ruleForm.youxiang" autocomplete="off" placeholder="邮箱"  />
			</el-form-item>
			<!-- <div v-if="tableName=='shiyanyuan'" class="input-group">
			   <div class="label">电话</div>
			...
			<el-form-item label="电话" class="input" v-if="tableName=='shiyanyuan'">
			  <el-input v-model="ruleForm.dianhua" autocomplete="off" placeholder="电话"  />
			</el-form-item>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if((!this.ruleForm.zhanghao) && `shiyanyuan` == this.tableName){
        this.$message.error(`账号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `shiyanyuan` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      if(`shiyanyuan` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮件格式`);
        return
      }
      if(`shiyanyuan` == this.tableName && this.ruleForm.dianhua&&(!this.$validate.isMobile(this.ruleForm.dianhua))){
        this.$message.error(`电话应输入手机格式`);
        return
      }
      this.$http({
